//Matteo created closingPriceBarChart ot show the closing price compared to the total time of the stock
// Matteo updated the class to implement 5 year, 2year, 1 year time periods and no data to display if there is no data
//Ruxin, fixed a bug that the silder of closing bar chart doesn't work,10am, 22/04
    ...
    if (data.size()>45) {
      ...
      for (int i = displayEnd-45; i < displayEnd; i++ ) {
        ...
        if (mouseY < height + y && mouseY > y &&
          ...
          fill(0);
          textFont(stdFont);
          //Tim, fixed error in displaying volume instead of closing price and 
